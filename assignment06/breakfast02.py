...
async def FryEggs(eggs):
    ...
    for egg in range(eggs):
        print(f"{time.ctime()} – Frying {egg + 1} egg(s)")
        await asyncio.sleep(1)
...
async def main():
    ...
    breackfast = [FryEggs(2),FryBacon(),ToastBread(2)]
    tasks = [asyncio.create_task(food) for food in breackfast]
    # eggs_task = asyncio.create_task(FryEggs(2)) # schedule execution
